...
def fpv_s_on_grid(cache_path: str, s_grid: np.ndarray, default_look: float = 200.0) -> np.ndarray:
    """Map each s_k to a farthest visible arclength s_fpv[k] using the precomputed cache.
    If a valid FPV is missing/behind, use a conservative fallback sk+default_look.
    """
    with open(cache_path, 'r', encoding='utf-8') as f:
        cache = json.load(f)
    entries = cache.get('entries', [])
    if not entries:
        return s_grid + default_look
    s0 = np.array([float(e.get('s0', 0.0)) for e in entries], dtype=float)
    sfv = np.array([
        ((e.get('final_fpv', {}) or {}).get('s', np.nan)) for e in entries
    ], dtype=float)
...
def build_vsafe_array(mode: str,
    ...
    if mode == 'curvature':
        if s_fpv is None:
            ...
            # map s_fpv to nearest index
            k_idx = np.clip(np.searchsorted(s_grid, s_fpv), 0, len(s_grid)-1)
            k_use = np.abs(kappa[k_idx])
...
def generate_reference_path(
    ...
    fwl = interp1d(s_raw, updated_path.dist_to_left_bound,  kind='linear')
...
def visualize_track(
    ...
    x_left  = new_ref_path.x_coordinates - new_ref_path.dist_to_left_bound  * np.sin(theta)
    y_left  = new_ref_path.y_coordinates + new_ref_path.dist_to_left_bound  * np.cos(theta)
    # Right edge: r - wR * n_hat
    x_right = new_ref_path.x_coordinates + new_ref_path.dist_to_right_bound * np.sin(theta)
    y_right = new_ref_path.y_coordinates - new_ref_path.dist_to_right_bound * np.cos(theta)
...
def solve_min_time(
    ...
    for k in range(N):
        ...
        # Frenet 横向界：绿色规划带 (road ∩ FOV ∩ Unknown)
        n_min_k = float(nMinPlan[min(k+1, len(s)-1)])
        n_max_k = float(nMaxPlan[min(k+1, len(s)-1)])
