...
def vehicle_dynamics(x, u, kappa):
    ...
    # 参数 
    m = 1200.0    # 车辆质量
    Jzz = 1260.0  # 车辆绕z轴的转动惯量
    lf, lr = 1.5, 1.4 # 前轴和后轴到重心的距离
    wf, wr_ = 1.6, 1.5  # 前轮和后轮的宽度
    g = 9.81 # 重力加速度
    Fz = m * g / 4  # 每个轮胎的垂直载荷
    delta_r = 0.0  
    epsilon = 1e-4 
...
# 轮胎模型
    ...
    def Fy(alpha):
        ...
        return 1000 * alpha  # 线性化模型
        
    FY_fl = Fz * Fy(alpha_fl) # 前左轮侧向力
    FY_fr = Fz * Fy(alpha_fr) # 前右轮侧向力
    FY_rl = Fz * Fy(alpha_rl) # 后左轮侧向力
    FY_rr = Fz * Fy(alpha_rr) # 后右轮侧向力
...
def solve_min_time(csv_file='Nuerburgring.csv', ds=2):
    ...
    F_z = m * g / 4 # 每个轮胎的垂直载荷
    mu = 0.9 # 摩擦系数
    F_x_max = 7100.0 / 2 # 每个轮胎的最大纵向力
    F_y_max = mu * F_z # 每个轮胎的最大侧向力
    a_x_max = mu * g # 每个轮胎的最大纵向加速度
    a_y_max = mu * g
    ...
    for k in range(N):
        ...
        w += [Uk]
        w0 += [0.0, 500.0] # 初始状态
        lbw += [-np.pi/6, -21000.0] # 控制输入下限
        ubw += [np.pi/6, 7100.0] # 控制输入上限
